...
# send notification to qiye weixin
function qywx_notify_single() {
    ACTION_NAME=`parse_action_label "$1"`
    QYWX_KEY=$2
    ...
    curl --speed-time 3 --speed-limit 1 "https://qyapi.weixin.qq.com/cgi-bin/webhook/send?key=${QYWX_KEY}" \
        -H "Content-Type: application/json" \
        -d '{
        "msgtype": "markdown",
        "markdown": {
            "content": "#### 【'"$APP_NAME"'】 '"$ACTION_NAME"'. \n> Branch：'"$(parse_git_branch)"' \n\n> Commit Msg：'"$(parse_git_message)"'\n\n> Commit ID: '"$(parse_git_hash)"'\n\n'"${elasped_lable}"'"
        }
        }' > /dev/null 2>>/var/log/webhook/notify.log

    echo -e "$APP_NAME $ACTION_NAME. QYWX Notification Sended."
}
...
function notify(){
    ...
    notify_run "qywx_notify_single" "$1" "$QYWX_BOT_KEY_LIST"
}
